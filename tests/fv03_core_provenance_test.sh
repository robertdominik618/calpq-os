...
fail() { printf 'FV03 CORE PROVENANCE: %s\n' "$1" >&2; exit 1; }

phase="$(bash scripts/governance_lifecycle_phase.sh)"
[[ "$phase" == "POST_FV00_IMPLEMENTATION" ]] \
  || fail "FV-03 tests require POST_FV00_IMPLEMENTATION, got $phase"
...
test_count="$(grep -Ec "^test\\('FV03-[0-9]{2}" packages/core/test/fv03-provenance.test.ts)"
[[ "$test_count" -eq 20 ]] || fail "expected exactly 20 mandatory FV-03 runtime tests, got $test_count"

node --test packages/core/test/fv03-provenance.test.ts
...
jq -e '((.dependencies // {}) | length) == 0 and ((.optionalDependencies // {}) | length) == 0 and ((.peerDependencies // {}) | length) == 0' \
  ...
  || fail '@calpq/core must remain runtime-dependency free in FV-03'
...
if grep -R -nE "from ['\"](react|react-native|expo|fastify|@?prisma|typeorm|sequelize|knex|drizzle|aws-sdk|@aws-sdk|openai|@anthropic-ai|tesseract|firebase)" \
  ...
  fail 'forbidden framework/provider dependency imported into FV-03 Core surface'
fi
...
printf 'FV03 CORE PROVENANCE: PASS / 20 TESTS / IMMUTABLE EVIDENCE + PROVENANCE / OUTCOME-ERROR SEPARATION\n'
