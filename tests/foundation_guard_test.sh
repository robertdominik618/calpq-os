...
run_guard() {
  (
    cd "$fixture/work"
    bash scripts/foundation_guard.sh
  )
}
...
expect_reject "dependency manifest before stack approval" run_guard

copy_fixture
touch "$fixture/work/rogue.ts"
expect_reject "product source file while frozen" run_guard
