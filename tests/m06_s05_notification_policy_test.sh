...
if grep -En 'Date\.now\(|new Date\(|Math\.random\(|randomUUID\(|setTimeout\(|setInterval\(|fetch\(|from .node:|from .*(react|openai|anthropic|axios|nodemailer|twilio)|sendMail\(|sendSms\(|pushNotification\(' "$source"; then
  echo 'M06 S05 ambient-time/provider/scheduler boundary violated' >&2
  exit 1
fi
...
verifySource('packages/application/test/m06-s05-notification-policy.test.ts','M06S05',96);
verifySource('tests/m06_s05_scope_test.mjs','M06S05G',16);
...
NODE
printf 'M06 S05: 96/96 runtime, 16/16 scope and 18 readonly assertions PASS.\n'
...
verifyTap(process.argv[4],'M06S02',72);
verifyTap(process.argv[5],'M06S01',64);
NODE
