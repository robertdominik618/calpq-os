...
fail() { printf 'FV02 CORE PORTS: %s\n' "$1" >&2; exit 1; }

phase="$(bash scripts/governance_lifecycle_phase.sh)"
[[ "$phase" == "POST_FV00_IMPLEMENTATION" ]] \
  || fail "FV-02 tests require POST_FV00_IMPLEMENTATION, got $phase"
...
test_count="$(grep -Ec "^test\\('FV02-[0-9]{2}" packages/core/test/fv02-ports.test.ts)"
[[ "$test_count" -eq 12 ]] || fail "expected exactly 12 mandatory FV-02 runtime tests, got $test_count"

node --test packages/core/test/fv02-ports.test.ts
...
jq -e '((.dependencies // {}) | length) == 0 and ((.optionalDependencies // {}) | length) == 0 and ((.peerDependencies // {}) | length) == 0' \
  ...
  || fail '@calpq/core must remain runtime-dependency free in FV-02'
...
if grep -R -nE "from ['\"](react|react-native|expo|fastify|@?prisma|typeorm|sequelize|knex|drizzle|aws-sdk|@aws-sdk|openai|@anthropic-ai|tesseract|firebase)" \
  ...
  fail 'forbidden framework/provider dependency imported into FV-02 Core surface'
fi

if grep -R -nE 'Date\.now\(|new Date\(\)|Math\.random\(|randomUUID\(|crypto\.randomUUID\(' \
  packages/core/src packages/core/test-support --include='*.ts' >/dev/null; then
  fail 'global wall-clock/randomness/UUID generation detected in deterministic Core or test support'
fi

if grep -R -nE '\b(CredentialArtifact|EligibilityAssessment|AuthorizationGrant)\b' \
  packages/core/src/ports packages/core/test-support --include='*.ts' >/dev/null; then
  fail 'later-phase domain capability leaked into FV-02-owned source'
fi

bash tests/fv01_core_primitives_test.sh >/dev/null
bash tests/architecture_boundaries_test.sh >/dev/null

printf 'FV02 CORE PORTS: PASS / 12 TESTS / CLOCK + ID PORTS / DETERMINISTIC FAKES / FV01 REGRESSION GREEN\n'
