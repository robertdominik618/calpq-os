...
if grep -R -nE 'ADMITTED / IMPLEMENTATION AUTHORIZED|ADMITTED_FOR_IMPLEMENTATION' \
  docs/planning/M0{4,5,6,7,8}_EXECUTION_PACKAGE.md >/dev/null; then
  fail 'later milestone admission leaked into M03 transition'
fi
...
printf 'M03 ADMISSION: PASS / M02 REVIEWED MERGE VERIFIED / M03 SLICE 01 AUTHORIZED / M04-M08 BLOCKED\n'
