...
function correctedExpiry(value: string | number | boolean | null = '2029-09-17') { return ExtractionFieldReviewDecision.corrected({ fieldProposal: expiryField, correctedValue: value, reason: 'Visible expiry differs from extraction' }); }
...
test('M05S04-22 USER_CORRECTED allows confirmed plus corrected', () => assert.equal(append(ExtractionReviewState.USER_CORRECTED,[confirmed(), correctedExpiry()]).currentState, ExtractionReviewState.USER_CORRECTED));
...
test('M05S04-43 serialization is deterministic', () => { const a=append(ExtractionReviewState.USER_CORRECTED,[confirmed(),correctedExpiry()]); const b=append(ExtractionReviewState.USER_CORRECTED,[correctedExpiry(),confirmed()]); assert.deepEqual(a.toJSON(),b.toJSON()); });
