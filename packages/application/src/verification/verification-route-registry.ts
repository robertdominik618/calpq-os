...
const ASSURANCE_RANK: Readonly<Record<VerificationAssuranceLevel, number>> = Object.freeze({
  BASIC: 1,
  SUBSTANTIAL: 2,
  HIGH: 3,
});
...
export class VerificationRouteDefinition {
  ...
  static create(input: {
    ...
    if (!Number.isSafeInteger(input.priority) || input.priority < 0 || input.priority > 100000) {
      throw new RangeError('Verification route priority must be a non-negative safe integer');
    }
    if (!Array.isArray(input.jurisdictions) || input.jurisdictions.length === 0 || input.jurisdictions.some((value) => !(value instanceof Jurisdiction))) {
      throw new TypeError('Verification route requires controlled jurisdictions');
    }
    const jurisdictions = [...input.jurisdictions];
    if (new Set(jurisdictions.map((value) => value.toString())).size !== jurisdictions.length) {
      throw new TypeError('Verification route jurisdictions must be unique');
    }
    ...
    if (!(input.sourceId instanceof SourceId) || !(input.sourceVersion instanceof VersionId) || !(input.retrievedAt instanceof UtcInstant)) {
      throw new TypeError('Verification route requires source/version/retrieval provenance');
    }
    return new VerificationRouteDefinition({
      id: input.id,
      method: input.method,
      capability: requiredText(input.capability, 'Verification capability', 256),
      verifierEntity: input.verifierEntity,
      authorityRole: input.authorityRole,
      supportedClaims: uniqueSortedText(input.supportedClaims, 'Supported verification claim'),
      ...
      sourceSnapshotReference: requiredText(input.sourceSnapshotReference, 'Route source snapshot reference', 512),
  ...
  isEffectiveAt(instant: UtcInstant): boolean {
    ...
    return this.state === VerificationRouteState.ACTIVE && isEffective(this.validFrom, this.validTo, instant);
  }

  supportsClaim(claim: string): boolean {
    return this.supportedClaims.includes(requiredText(claim, 'Verification claim', 512));
  }

  toJSON() {
    return Object.freeze({
      id: this.id.toString(), method: this.method, capability: this.capability,
      verifierEntityId: this.verifierEntity.id.toString(), authorityRole: this.authorityRole,
      supportedClaims: Object.freeze([...this.supportedClaims]),
      jurisdictions: Object.freeze(this.jurisdictions.map((value) => value.toString())),
      assuranceLevel: this.assuranceLevel, priority: this.priority, state: this.state,
      validFrom: this.validFrom.toString(), validTo: this.validTo?.toString() ?? null,
      sourceId: this.sourceId.toString(), sourceVersion: this.sourceVersion.toString(),
      sourceSnapshotReference: this.sourceSnapshotReference, retrievedAt: this.retrievedAt.toString(),
    });
  }
}
...
export class VerificationRouteRegistrySnapshot {
  ...
  private constructor(input: {
    ...
    this.id = input.id;
    this.trustSnapshot = input.trustSnapshot;
  ...
  static create(input: {
    ...
    if (!(input.asKnownAt instanceof UtcInstant) || input.asKnownAt.toEpochMilliseconds() > input.trustSnapshot.asKnownAt.toEpochMilliseconds()) {
      throw new RangeError('Route registry asKnownAt must not exceed Trust Registry knowledge cutoff');
    }
    if (!(input.policyVersion instanceof VersionId)) throw new TypeError('Route registry requires policy VersionId');
    if (!Array.isArray(input.routes) || input.routes.some((value) => !(value instanceof VerificationRouteDefinition))) {
      throw new TypeError('Route registry routes must use VerificationRouteDefinition');
    }
    ...
    for (const route of routes) {
      ...
      if (route.retrievedAt.toEpochMilliseconds() > input.asKnownAt.toEpochMilliseconds()) {
        throw new RangeError('Route retrieved after registry knowledge cutoff is not yet known');
      }
    }
    routes.sort((a, b) => a.priority - b.priority || a.id.toString().localeCompare(b.id.toString()));
    return new VerificationRouteRegistrySnapshot({
      id: input.id, trustSnapshot: input.trustSnapshot, asKnownAt: input.asKnownAt,
      routes: Object.freeze(routes), policyVersion: input.policyVersion,
    });
  }

  toJSON() {
    return Object.freeze({
      id: this.id.toString(), trustSnapshotId: this.trustSnapshot.id.toString(), asKnownAt: this.asKnownAt.toString(),
      policyVersion: this.policyVersion.toString(), routes: Object.freeze(this.routes.map((value) => value.toJSON())),
    });
  }
}
...
export class VerificationRequest {
  ...
  private constructor(input: VerificationRequest) {
    Object.assign(this, input);
    Object.freeze(this);
  }
  ...
  static create(input: {
    ...
    if (!Array.isArray(input.acceptableMethods) || input.acceptableMethods.length === 0 || input.acceptableMethods.some((value) => !VERIFICATION_METHODS.has(value))) {
      throw new TypeError('Verification request requires controlled acceptable methods');
    }
    ...
    return new VerificationRequest({
      id: input.id, evidence: input.evidence,
      subjectReference: requiredText(input.subjectReference, 'Verification subject reference', 512),
      claims: uniqueSortedText(input.claims, 'Verification claim'),
      jurisdiction: input.jurisdiction,
      useCase: requiredText(input.useCase, 'Verification use case', 512),
      requiredAssurance: input.requiredAssurance,
      acceptableMethods: Object.freeze([...input.acceptableMethods].sort()),
      evaluationInstant: input.evaluationInstant, asKnownAt: input.asKnownAt,
      idempotencyKey: requiredText(input.idempotencyKey, 'Verification idempotency key', 512),
    } as VerificationRequest);
  }

  toJSON() {
    return Object.freeze({
      id: this.id.toString(), evidenceId: this.evidence.id.toString(), subjectReference: this.subjectReference,
      claims: Object.freeze([...this.claims]), jurisdiction: this.jurisdiction.toString(), useCase: this.useCase,
      requiredAssurance: this.requiredAssurance, acceptableMethods: Object.freeze([...this.acceptableMethods]),
      evaluationInstant: this.evaluationInstant.toString(), asKnownAt: this.asKnownAt.toString(), idempotencyKey: this.idempotencyKey,
    });
  }
}
...
export class VerificationRouteSelectionItem {
  ...
  constructor(route: VerificationRouteDefinition, claims: readonly string[], authorityResolutions: readonly AuthorityResolutionResult[]) {
    ...
    this.authorityResolutions = Object.freeze([...authorityResolutions]);
    Object.freeze(this);
  }
  toJSON() {
    return Object.freeze({ routeId: this.route.id.toString(), claims: Object.freeze([...this.claims]), authorityResolutionIds: Object.freeze(this.authorityResolutions.map((value) => value.id.toString())) });
  }
}

function isSufficientAuthority(outcome: AuthorityResolutionOutcome): boolean {
  return outcome === AuthorityResolutionOutcome.AUTHORIZED || outcome === AuthorityResolutionOutcome.AUTHORIZED_WITH_CONDITIONS;
}
...
function authorityMatches(route: VerificationRouteDefinition, request: VerificationRequest, claim: string, resolution: AuthorityResolutionResult): boolean {
  return resolution.entity === route.verifierEntity &&
    resolution.requestedRole === route.authorityRole &&
    resolution.claimScope === claim &&
    resolution.jurisdiction.toString() === request.jurisdiction.toString() &&
    resolution.evaluationInstant.toEpochMilliseconds() === request.evaluationInstant.toEpochMilliseconds() &&
    resolution.asKnownAt.toEpochMilliseconds() === request.asKnownAt.toEpochMilliseconds();
}
...
export class VerificationRouteSelection {
  ...
  private constructor(input: { readonly registry: VerificationRouteRegistrySnapshot; readonly request: VerificationRequest; readonly outcome: VerificationSelectionOutcome; readonly items: readonly VerificationRouteSelectionItem[]; readonly uncoveredClaims: readonly string[]; readonly reasonCodes: readonly string[] }) {
    Object.assign(this, input);
    Object.freeze(this);
  }
  ...
  static select(input: { readonly registry: VerificationRouteRegistrySnapshot; readonly request: VerificationRequest; readonly authorityResolutions: readonly AuthorityResolutionResult[] }): VerificationRouteSelection {
    ...
    for (const resolution of input.authorityResolutions) {
      if (resolution.snapshot !== input.registry.trustSnapshot) throw new TypeError('Authority resolution must bind exact Trust Registry snapshot');
    }

    const items: VerificationRouteSelectionItem[] = [];
    let reviewSignal = false;
    for (const route of input.registry.routes) {
      if (!route.isEffectiveAt(input.request.evaluationInstant)) continue;
    ...
    let outcome: VerificationSelectionOutcome;
  ...
  toJSON() {
    return Object.freeze({ registrySnapshotId: this.registry.id.toString(), requestId: this.request.id.toString(), outcome: this.outcome, items: Object.freeze(this.items.map((value) => value.toJSON())), uncoveredClaims: Object.freeze([...this.uncoveredClaims]), reasonCodes: Object.freeze([...this.reasonCodes]) });
  }
}
...
export class VerificationProviderRequest {
  ...
  private constructor(input: VerificationProviderRequest) { Object.assign(this, input); Object.freeze(this); }
  ...
  static create(input: { readonly attemptId: VerificationAttemptId; readonly route: VerificationRouteDefinition; readonly request: VerificationRequest; readonly claims: readonly string[]; readonly idempotencyKey: string }): VerificationProviderRequest {
    ...
    return new VerificationProviderRequest({ attemptId: input.attemptId, route: input.route, request: input.request, claims, idempotencyKey: requiredText(input.idempotencyKey, 'Provider idempotency key', 512) } as VerificationProviderRequest);
  }
}
...
export class VerificationProviderResult {
  ...
  private constructor(input: VerificationProviderResult) { Object.assign(this, input); Object.freeze(this); }
  ...
  static create(input: { readonly outcome: VerificationRouteOutcome; readonly adapterReference: string; readonly adapterVersion: string; readonly checkedClaims: readonly string[]; readonly assertionFingerprints?: Readonly<Record<string, string>>; readonly sourceSnapshotReference: string; readonly sourceVersionReference: string; readonly checkedAt: UtcInstant; readonly providerResultReference?: string | null; readonly reasonCodes?: readonly string[] }): VerificationProviderResult {
    ...
    const reasons = Object.freeze([...(input.reasonCodes ?? [])].map((value) => requiredText(value, 'Provider reason code', 256)).sort());
    if (new Set(reasons).size !== reasons.length) throw new TypeError('Provider reason codes must be unique');
    if (input.outcome === VerificationRouteOutcome.FAILED && reasons.some((value) => value === 'PROVIDER_UNAVAILABLE' || value === 'PROVIDER_TIMEOUT')) {
      throw new TypeError('Provider outage/timeout must be INDETERMINATE, never FAILED');
    }
    ...
    return new VerificationProviderResult({
      ...
      adapterReference: requiredText(input.adapterReference, 'Adapter reference', 256),
      adapterVersion: requiredText(input.adapterVersion, 'Adapter version', 256),
      checkedClaims,
      assertionFingerprints: Object.freeze(fingerprints),
      sourceSnapshotReference: requiredText(input.sourceSnapshotReference, 'Provider source snapshot reference', 512),
      sourceVersionReference: requiredText(input.sourceVersionReference, 'Provider source version reference', 512),
      checkedAt: input.checkedAt,
      providerResultReference: input.providerResultReference == null ? null : requiredText(input.providerResultReference, 'Provider result reference', 512),
      reasonCodes: reasons,
    } as VerificationProviderResult);
  }
}
...
export class VerificationRouteResult {
  ...
  private constructor(input: { readonly providerRequest: VerificationProviderRequest; readonly providerResult: VerificationProviderResult; readonly authorityResolutions: readonly AuthorityResolutionResult[]; readonly outcome: VerificationRouteOutcome; readonly checkedClaims: readonly string[]; readonly reasonCodes: readonly string[] }) {
  ...
  static normalize(input: { readonly providerRequest: VerificationProviderRequest; readonly providerResult: VerificationProviderResult; readonly authorityResolutions: readonly AuthorityResolutionResult[] }): VerificationRouteResult {
    ...
    let outcome = input.providerResult.outcome;
    const reasons = [...input.providerResult.reasonCodes];
  ...
  assertionFingerprintFor(claim: string): string | null {
    return this.providerResult.assertionFingerprints[claim] ?? null;
  }

  toJSON() {
    return Object.freeze({
      attemptId: this.providerRequest.attemptId.toString(), routeId: this.providerRequest.route.id.toString(), requestId: this.providerRequest.request.id.toString(),
      outcome: this.outcome, checkedClaims: Object.freeze([...this.checkedClaims]), adapterReference: this.providerResult.adapterReference,
      adapterVersion: this.providerResult.adapterVersion, sourceSnapshotReference: this.providerResult.sourceSnapshotReference,
      sourceVersionReference: this.providerResult.sourceVersionReference, checkedAt: this.providerResult.checkedAt.toString(),
      providerResultReference: this.providerResult.providerResultReference, authorityResolutionIds: Object.freeze(this.authorityResolutions.map((value) => value.id.toString())),
...
export class VerificationClaimResolution {
  ...
  private constructor(input: { readonly claim: string; readonly outcome: VerificationRouteOutcome; readonly routeResults: readonly VerificationRouteResult[]; readonly assertionFingerprint: string | null; readonly reasonCodes: readonly string[] }) {
    Object.assign(this, input); Object.freeze(this);
  }
  ...
  static resolve(claimInput: string, resultsInput: readonly VerificationRouteResult[]): VerificationClaimResolution {
    ...
    let outcome: VerificationRouteOutcome;
  ...
  toJSON() {
    return Object.freeze({ claim: this.claim, outcome: this.outcome, routeResultAttemptIds: Object.freeze(this.routeResults.map((value) => value.providerRequest.attemptId.toString())), assertionFingerprint: this.assertionFingerprint, reasonCodes: Object.freeze([...this.reasonCodes]) });
  }
}
