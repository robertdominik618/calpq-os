...
test('M04S08-35-duplicate-source-id-with-different-version-is-rejected', () => { const x = single(DomainOutcome.SATISFIED); assert.throws(() => graph(x.valueGap, { sources: [source(), source({ version: 'source-2' })] }), /at most one exact SourceReference version/i); });
