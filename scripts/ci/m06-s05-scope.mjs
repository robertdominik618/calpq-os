...
export function dispatchPatch(original){
  ...
  assert.equal(original.split(marker).length,2);assert(original.endsWith(tail),'Exact S04 terminal CLI required');
  const prefix=original.slice(0,-tail.length).replace(marker,"import {mkdtempSync,rmSync,existsSync} from 'node:fs';");
